    ...
    <form class="myform" method="POST" action="{{ route('admin.projects.store') }}">
        ...
        <div class="row mt-3">
            ...
            <div class="col-6">
                ...
                <input type="text" maxlength="255" name="tags" class="form-control @error('tags') is-invalid @enderror"
                    value="{{ old('tags') }}" placeholder="Inserisci tag del progetto">
                @error('tags')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="row d-flex justify-content-center mt-3">
            <div class="col-8">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" id="editor" name="description"
                    rows="8" cols="30"
                    placeholder="Inserisci una breve descrizione del progetto">{{ old('description') }}</textarea>
                @error('description')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
